...
var deployDependencies = ["archiver", "aws-sdk", "chalk",
    "debug", "tmp", "yargs"];
  ...
  createLambda(role, llamaFile, function(err, data) {
    ...
    if (llamaFile) {
      ...
      configSchedule(interval, llamaConfig.FunctionArn, function(err) {
        ...
        if (type === 'rate') {
          console.log(chalk.green(util.format('+ Chaos Llama\'s schedule updated to %s', interval)));
        } else {
          console.log(chalk.green(util.format('+ Chaos Llama\'s schedule updated to the cron expression', interval)));
        }
...
function createEventRule(context, next) {
  ...
  var params = {
    ...
    ScheduleExpression: context.interval,
    State: 'ENABLED'
  };
